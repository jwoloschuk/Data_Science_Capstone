...
summaryDataset <- data.frame( KEY = character(), SIZE = character(), LINES = character(), CHARACTERS = character(), WORDS = character(),LONGEST = character(), stringsAsFactors = FALSE)

# Call function and add Twitter data to empty dataframe
summaryDataset <- bind_rows(summaryDataset, Data_Summary_Function( Twitter_File_Name, Twitter_File_Path, Twitter_data) )

# Call function and add News data to empty dataframe
summaryDataset <- bind_rows(summaryDataset, Data_Summary_Function( News_File_Name, News_File_Path, News_data) )

# Call function and add Blog data to empty dataframe
summaryDataset <- bind_rows(summaryDataset, Data_Summary_Function( Blogs_File_Name, Blogs_File_Path, Blogs_data) )

# Change the names of the rows/columns so they are more informative 
names(summaryDataset) <- c("File Name", "Size (MB)", "No. of Lines", "Number of Characters", "Number of Words","Longest Line Length")
...
We create a corpus To provide a uniform heterogeneous way of accessing the root TEXT of a document to perform text mining work, regardless of file location, internal annotations and other features unique to the document, we load up the text document(s) into a **"CORPUS"** from various **sources**.
...
Combined_Sample_Data <- c( Sample_Data_Function( Blogs_data ), Sample_Data_Function( News_data ), Sample_Data_Function( Twitter_data ) )
...
*  Remove stopwords, such as "in", "the", "a", "of", "and", etc.tasks performed to preprocess our various corpus.
...
barplot(head(Corpus_Frequency,20),main="Top 20 Highest Word Frequency", 
...
bigram <- removeSparseTerms(TermDocumentMatrix(Corpus_Processed_Data, control = list(tokenize = Bigram_Tokenizer_Function)), 0.9999)
bigram_frequency <- Frequency_Function(bigram)


# Call Trigram and Frequency functions 
trigram <- removeSparseTerms(TermDocumentMatrix(Corpus_Processed_Data, control = list(tokenize = Trigram_Tokenizer_Function)), 0.9999)
